...
description: Build and run a localized WeShop web form from JSON/YAML embedded in prose, including the minimal z-image agentName format, while securely handling an API key, bootstrapping Node.js and the WeShop CLI, and displaying generated images or videos. Use when a user wants a one-message, fixed-model WeShop generation UI on any Agent Skills-compatible host; use weshop-cli-skill for direct CLI-only generation.
metadata:
  version: "1.2.0"
...
The shortest supported request selects the fixed `z-image` form with exactly three non-secret fields plus a recognized API-key field:
...
Treat `safeGenerat` and `resultBase64` as strict launcher-contract markers, not CLI flags: require exactly the values above, remove the API key in memory, and map the request to the immutable `z-image` preset. Never forward either marker to the CLI.
...
1. Locate the JSON or supported YAML request. Read [references/config-schema.md](references/config-schema.md) for both accepted input modes and the preset list. The minimal mode requires `agentName: z-image`, `safeGenerat: off`, and boolean `resultBase64: true`; it maps only to the fixed `z-image` form. The general mode must select `formPreset`; it may set `defaults` for fields already defined by that preset, but it may not supply `command`, `fields`, `fixedArgs`, labels, flags, or enums. Recognize the API key only at these paths: top-level `apiKey`, `weshopApiKey`, or `WESHOP_API_KEY`; or the same names inside `secrets` or `env`.
...
- In minimal agent mode, accept no non-secret keys beyond `agentName`, `safeGenerat`, and `resultBase64`; require their exact values and never reinterpret them as CLI flags.
