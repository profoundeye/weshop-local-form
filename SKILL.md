...
description: Install and launch a localized WeShop web form from a Git-linked compact request or JSON/YAML embedded in prose, including the minimal z-image agentName format, while securely handling an API key and bootstrapping dependencies. Use for a one-message fixed-model WeShop UI on any Agent Skills-compatible host; use weshop-cli-skill for direct CLI-only generation.
metadata:
  version: "1.3.0"
...
1. Locate the compact Git-linked request, JSON, or supported YAML. Read [references/config-schema.md](references/config-schema.md) for accepted input modes and the preset list. The shortest mode requires the repository URL, `agentName: z-image`, `safeGenerat: off`, and an API key; it maps only to the fixed `z-image` form. `resultBase64: true` is optional compatibility input. The general mode must select `formPreset`; it may set `defaults` for fields already defined by that preset, but it may not supply `command`, `fields`, `fixedArgs`, labels, flags, or enums. Recognize the API key only at these paths: top-level `apiKey`, `weshopApiKey`, or `WESHOP_API_KEY`; or the same names inside `secrets` or `env`.
...
- In minimal agent mode, require `agentName: z-image` and `safeGenerat: off`. Accept optional `resultBase64: true` only for compatibility; never reinterpret these markers as CLI flags.
